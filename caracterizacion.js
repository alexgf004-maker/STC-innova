...
function renderLista() {
  ...
  // "Hecha, falta revisar" agrupada por DÍA (Hoy, Ayer, fechas), para que
  // el admin sepa cuándo se hizo cada una y confirme guiándose por el día.
  const seccionPorDia = (titulo, arr, color) => {
    if (!arr.length) return '';
    // Agrupar por clave de día (usa fechaHecha; sin fecha va a "Sin fecha")
    const grupos = {};
    for (const o of arr) {
      const clave = o.fechaHecha ? claveDia(o.fechaHecha) : 'sin-fecha';
      (grupos[clave] = grupos[clave] || []).push(o);
    }
    // Ordenar días del más reciente al más antiguo; "sin-fecha" al final
    ...
    for (const clave of claves) {
      const etq = clave === 'sin-fecha' ? 'Sin fecha' : etiquetaDia(clave);
      html += `
        <div style="display:flex;align-items:center;gap:6px;margin:10px 0 6px">
          <div style="font-size:11px;font-weight:700;color:var(--text-3)">${etq}</div>
          <div style="font-size:10px;color:var(--text-4)">· ${grupos[clave].length}</div>
        </div>
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:10px">${grupos[clave].map(tarjetaOrden).join('')}</div>`;
    }
    return html;
  };

  el.innerHTML = seccionPorDia('Hecha, falta revisar', porConfirmar, '#fbbf24')
...
function tarjetaOrden(o) {
  ...
  const detalle = (porConfirmar || confirmada) ? `
    ...
    ${(o.fechaHecha || o.hechaPor) ? `<div style="display:flex;align-items:center;gap:5px;margin-top:6px;font-size:10px;color:var(--text-4)">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" width="11" height="11"><circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 2"/></svg>
      ${o.fechaHecha ? fmtFechaHora(o.fechaHecha) : ''}${o.hechaPor ? ' · ' + o.hechaPor : ''}
    </div>` : ''}` : '';
